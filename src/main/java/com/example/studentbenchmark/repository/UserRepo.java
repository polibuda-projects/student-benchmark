...
public interface UserRepo extends JpaRepository<AppUser, Long> {
    ...
    @Query(nativeQuery = true, value= "SELECT * FROM App_User u WHERE u.idUser= :idUser")
    AppUser findByID(@Param("idUser") Long idUser);

    @Transactional
    @Modifying
    @Query(nativeQuery = true, value= "UPDATE App_User u SET u.password= :newPassword WHERE u.idUser= :idUser")
